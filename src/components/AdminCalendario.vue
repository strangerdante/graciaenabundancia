...
import { calendario } from "../lib/api";
import AdminCalendarioForm from "./AdminCalendarioForm.vue";
...
const loadEvents = async () => {
  try {
    ...
    const response = await calendario.getAll();
...
const handleCreate = async (eventData) => {
  try {
    error.value = "";
    await calendario.create(eventData);
...
const handleUpdate = async (eventData) => {
  ...
  try {
    ...
    await calendario.update(editingEvent.value.id, updatedData);
    formMode.value = "closed";
